...
#region:    Scan
    ...
    foreach ($index in $CheckValue){
        ...
        try{
            $results = Invoke-Command -ComputerName $computer -ScriptBlock {
                ...
                $property = $args[2]
                $value    = $args[3]
                (Get-ProcessMitigation -Name $appName -WarningAction SilentlyContinue -ErrorAction stop).$class.$property
            } -ArgumentList $appName, $class, $property, $value -ErrorAction stop
        } catch {
    ...
    if(($resultsArray -match "Fail").Count -gt 0){
        $passFail = "Fail"
    }elseif(($resultsArray -match "NULL").Count -gt 0){
        $passFail = "Fail"
