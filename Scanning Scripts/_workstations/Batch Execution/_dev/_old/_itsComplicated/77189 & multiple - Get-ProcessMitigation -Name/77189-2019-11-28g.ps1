...
#region: PassSummary loop
    ...
    foreach ($line in $Vul_ID_Test_Results){
        if ($line.Pass_Fail -eq "PASS" ){
        
            $obj = [PSCustomObject]@{
                Computer = $line.computer
                Status   = $line.status                          
                Vul_ID   = $line.Vul_ID 
                Result   = $line.Pass_Fail
            }
            $PassSummary += $obj
        }
    }
...
""
#"results"
#Write-PSObject $results -MatchMethod Exact  -Value "FAIL" -ValueForeColor Red
""
#"Vul_ID_Test_Results"
#Write-PSObject -Object $Vul_ID_Test_Results -MatchMethod Exact -Column * -Value "FAIL" -ValueForeColor Red;
""
"PassSummary:"
Write-PSObject -Object $PassSummary         -MatchMethod Exact -Column * -Value "PASS" -ValueForeColor Green;
