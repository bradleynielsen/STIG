...
# 2020.03.13 - a) auto open csv results b) add note for buffer size  c) added init region d) adding asjob
#
# ##########################################################################################################################>
...
#region:    Summary
    ...
    $summaryResults | 
        Export-Csv `
            -Path $scriptRootPath\"Summary - "$datestamp".csv" `
            -Force `
            -NoTypeInformation 

    & $scriptRootPath\"Summary - "$datestamp".csv"
...
#$host.UI.RawUI.BufferSize = New-Object System.Management.Automation.Host.Size(1000,0)
