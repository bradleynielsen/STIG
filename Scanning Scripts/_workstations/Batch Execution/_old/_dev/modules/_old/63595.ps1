...
$results = Invoke-Command -ComputerName $computer -ScriptBlock {
    Get-CimInstance -ClassName Win32_DeviceGuard -Namespace root\Microsoft\Windows\DeviceGuard| select RequiredSecurityProperties,VirtualizationBasedSecurityStatus 
} -ErrorAction SilentlyContinue

if(($results|select RequiredSecurityProperties,VirtualizationBasedSecurityStatus|Out-String) -eq ($CheckValue|Out-String)){
    $passFail = "Pass"
}else{
    $passFail = "Fail"
}
