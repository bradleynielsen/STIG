...
# 2020.05.09 - a) Removing update loop in 'Process Jobs'; jobs are still causing endless loop
# 
# ##########################################################################################################################>
...
#region:    Config
    ...
    $timeout    =  20 # [number seconds | 0: OFF] set the timeout for WinRM (prevents endless loop) 
    $jobTimeout =  3 # [number of Hours]         set the timeout for all jobs (Total time will not exeed this number) 
...
#region:    Summary
    ...
    $testingJobs | Wait-Job -timeout ($jobTimeout*60*60) |out-null # Total job timeout in hours
